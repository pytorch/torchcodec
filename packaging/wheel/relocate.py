...
def relocate_elf_library(patchelf, output_dir, output_library, binary):
    ...
    new_libraries_path = osp.join(output_dir, "torchcodec.libs")
    ...
    subprocess.check_output(
        [patchelf, "--set-rpath", "$ORIGIN:$ORIGIN/../torchcodec.libs", binary_path], cwd=output_library
    )
...
def patch_linux():
    ...
    binaries = [f"libtorchcodec{n}.so" for n in (4,5,6,7)]
    for wheel in wheels:
        ...
        print("Findingbinaries")
        output_library = osp.join(output_dir, "torchcodec")
